...
from deep_learning_larcc.recurrent.v1.create_models import (create_lstm_v1_0, create_lstm_v1_1, create_lstm_v1_2,
                                                            create_lstm_v1_3, create_lstm_v1_4, create_lstm_v1_5)
...
if __name__ == '__main__':
    ...
    # for n in track(range(n_times), total=n_times):
        training_data = np.load(ROOT_DIR + "/haptic_data/data1/global_normalized_train_data_20ms.npy")
        x_train = np.reshape(training_data[:, :-1], (training_data.shape[0], 20, 13))
        y_train = to_categorical(training_data[:, -1])
        x_train = x_train[:, :, 1:]

        model, model_name = create_lstm_v1_2()
