...
if __name__ == '__main__':
    ...
    with open("../../convolutional/dataset1_results/statistical_metrics_100_dataset1_v1_0.json", 'r', encoding='utf-8') as file_v1_0:
        data_v1_0 = json.load(file_v1_0)
    with open("../../convolutional/dataset1_results/statistical_metrics_100_dataset1_v1_1.json", 'r', encoding='utf-8') as file_v1_1:
        data_v1_1 = json.load(file_v1_1)
    with open("../../convolutional/dataset1_results/statistical_metrics_100_dataset1_v1_2.json", 'r', encoding='utf-8') as file_v1_2:
        data_v1_2 = json.load(file_v1_2)
    with open("../../convolutional/dataset1_results/statistical_metrics_100_dataset1_v1_3.json", 'r', encoding='utf-8') as file_v1_3:
    ...
    bars_v1_0 = ax.bar(x - (3*width/2), v1_0_mean, width, label='v1.0', color="#8f9bff", edgecolor="white",
           linewidth=2)
    ax.errorbar(x - (3*width/2), v1_0_mean, yerr=v1_0_std_dev, fmt='none',
                color="#8f9bff", ecolor="black", elinewidth=2, capsize=5, alpha= alpha)

    bars_v1_1 = ax.bar(x - (width/2), v1_1_mean, width, label='v1.1', color="#47cd4d", edgecolor="white",
           linewidth=2)  # Translucent bars
    ax.errorbar(x - (width/2), v1_1_mean, yerr=v1_1_std_dev, fmt='none',
    ...
    bars_v1_3 = ax.bar(x + (3*width/2), v1_3_mean, width, label='v1.3', color="#e6df07", edgecolor="white",
           linewidth=2)
    ax.errorbar(x + (3*width/2), v1_3_mean, yerr=v1_3_std_dev, fmt='none',
                color="#e6df07", ecolor="black", elinewidth=2, capsize=5, alpha= alpha)

    # Add the values on top of each main bar (not on error bars)
    for bars, values in zip([bars_v1_0, bars_v1_1, bars_v1_2, bars_v1_3],
                        [v1_0_mean, v1_1_mean, v1_2_mean, v1_3_mean]):
    ...
    ax.legend()
    ax.set_title("Mean & Standard Deviation - CNNs v1 - 100 simulations (train + dataset1 test)", fontsize=14,
                 fontweight='bold')

    # Show the plot
    fig.tight_layout()
    delta = 0.03
    plt.ylim([min(v1_0_mean + v1_1_mean + v1_2_mean + v1_3_mean) - delta,
              max(v1_0_mean + v1_1_mean + v1_2_mean) + delta])
    plt.show()
    ...
    for mtrc, met_title in [("accuracy", "Accuracy"), ("precision", "Precision"), ("recall", "Recall"),
        ...
        data = [
            ...
            ["CNN v1.0", data_v1_0[mtrc]["iqr"], data_v1_0[mtrc]["cv"], data_v1_0[mtrc][
                "95_confidence_interval"], ],
            ["CNN v1.1", data_v1_1[mtrc]["iqr"], data_v1_1[mtrc]["cv"], data_v1_1[mtrc][
                "95_confidence_interval"], ],
            ["CNN v1.2", data_v1_2[mtrc]["iqr"], data_v1_2[mtrc]["cv"], data_v1_2[mtrc][
                "95_confidence_interval"], ],
            ["CNN v1.3", data_v1_3[mtrc]["iqr"], data_v1_3[mtrc]["cv"], data_v1_3[mtrc][
                "95_confidence_interval"], ]
        ]
        pdf.create_table(table_data=data)
        pdf.ln()
        pdf.ln()
    pdf.output('CNNs_v1_statistical_metrics.pdf')