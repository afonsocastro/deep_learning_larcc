...
import numpy as np
from deep_learning_larcc.utils import NumpyArrayEncoder, prediction_classification
import json
...
def create_convolutional_nn(input):
    ...
    modelo.add(Conv2D(64, kernel_size=(13, 1), activation="relu", input_shape=(input, 12, 1)))
...
if __name__ == '__main__':
    ...
    for n in progressbar(range(n_times), redirect_stdout=True):
        ...
        x_train = np.reshape(training_data[:, :-1], (training_data.shape[0], input_nn, 13, 1))
        y_train = to_categorical(training_data[:, -1])
        x_train = x_train[:, :, 1:, :]

        test_data = np.load("../haptic_data/data1/global_normalized_test_data_500ms.npy")

        model = create_convolutional_nn(input_nn)

        # train the model
        fit_history = model.fit(x=x_train, y=y_train, validation_split=validation_split, epochs=500, verbose=2,
        ...
        for i in range(0, len(test_data)):
            ...
            prediction = model.predict(x=x_test[:, :, 1:, :], verbose=0)
        ...
        test_dict = {"pull": pull, "push": push, "shake": shake, "twist": twist}
    ...
    with open("dornas/x_time_window_train_test_" + str(time_window) + ".json", "w") as write_file:
